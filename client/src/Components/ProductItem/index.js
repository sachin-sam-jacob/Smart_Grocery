            ...
            postData(`/api/my-list/add/`, data).then((res) => {
                if (res.status !== false) {
                    context.setAlertBox({
                        open: true,
                        error: false,
                        msg: "The product has been added to my list"
                    });
        ...
        <>
            ...
            >
                                ...
                                <Slider {...settings} ref={sliderRef} className='productItemSlider'>
                                    {props.item?.images?.map((image, index) => (
                                        <div className='slick-slide' key={index}>
                                            <img src={image} className="w-100" alt={props.item.name} />
                                        </div>
                                    ))}
                                </Slider>
                            }
                ...
                <div className="info">
                    ...
                    {props.item?.productWeight && props.item.productWeight.length > 0 && (
                        <span style={{
                            fontWeight: '500',
                            fontSize: '14px',
                            color: '#555',
                            ...
                            Weight : {props.item.productWeight[0]}
                        </span>
                    )}
                </div>
            </div>
